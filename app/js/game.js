...
let start = false                                           // start
    diff_easy = false,                                      // for easy difficult (для легкой сложности)
    diff_medium = false,                                    // for medium difficult (для средней сложности)
    diff_hard = false,                                      // for hard difficult (для высокой сложности)
    screen_mode = 0,                                        // если 0, то 800x800, если 1, то на всесь экран
    spawnHeartInt  = 20000,                                 // spawn heart interval (базовый интервал для спавна хилок)
    menuThemeSwitch = false;                                // переключатедь музыки в меню, по умолч. вкл.             

let width = 800,                                            // width game field
    height = 800,                                           // height game field
    timer = 0;                                              // timer 
...
? spaceBackdround.src = './sprites/space-background.png'
: spaceBackdround.src = './sprites/space-background-16_9.png';                               


// loading all audio files
let menuTheme = loadAudio (
    ["./audio/menu/menu-sound.mp3", "./audio/menu/menu-sound.wav", "./audio/menu/menu-sound.ogg"], 
    0.2, 
    true, 
    false
);  
let battleTheme = loadAudio(
    ["./audio/battle/battle-sound.mp3", "./audio/battle/battle-sound.wav", "./audio/battle/battle-sound.ogg"], 
    0.1, 
    true
);
let shotSound = loadAudio(
    ["./audio/shot/shot-sound.mp3", "./audio/shot/shot-sound.wav", "./audio/shot/shot-sound.ogg"], 
    0.2
);
let explSound = loadAudio(
    ["./audio/expl/expl-sound.mp3", "./audio/expl/expl-sound.wav", "./audio/expl/expl-sound.ogg"], 
    0.2
);
let removeHealthSound = loadAudio(
    ["./audio/health/remove-health.mp3", "./audio/health/remove-health.wav", "./audio/health/remove-health.ogg"]
    , 0.2
);
let addHealthSound = loadAudio(
    ["./audio/health/add-health.mp3", "./audio/health/add-health.wav", "./audio/health/add-health.ogg"],
    0.4
);
let selectSound = loadAudio(
    ["./audio/select/select-sound.mp3", "./audio/select/select-sound.wav", "./audio/select/select-sound.ogg"],
    0.2
);
let gameOverSound = loadAudio(
    ["./audio/menu/game-over.mp3", "./audio/menu/game-over.wav", "./audio/menu/game-over.ogg"],
    0.4
);
...
function render() {
    ...
    //  create fire
    for (i in fire) {
        ctx.drawImage(bulletImg, fire[i].x, fire[i].y, fire[i].width, fire[i].height);
    }
    // draw player ship
    ctx.drawImage(playerShipImg, playerShip.x, playerShip.y, playerShip.width, playerShip.height);
    //  create asteroid
    for (i in astero) {
        ctx.drawImage(asteroImg, astero[i].x, astero[i].y, astero[i].width, astero[i].height);
    }
    //  create heart
    for (i in hearts) {
        ctx.drawImage(healthImg, hearts[i].x, hearts[i].y, hearts[i].width, hearts[i].height);
    }
    // draw expl
    for (i in expl) {
        ctx.drawImage(explImg, 
        170*Math.floor(expl[i].animX), 
        170*Math.floor(expl[i].animY), 
        170, 
        170,
        expl[i].x,
        expl[i].y,
    ...
    // draw health
    for (i in health) {
        ctx.drawImage(healthImg, health[i].x, health[i].y, health[i].width, health[i].height);
    }
    // score
...
? localStorage.getItem('record_score') 
: 0;                                                        
...
// bullet (пуля)
let fireDX = 0,                                             // change bullet x (изменение положение пули по x)
    fireDY = -5,                                            // change bullet y (изменение положение пули по y)
    fireWidth = 10,                                         // width bullet (ширина пули)
    fireHeight = 45,                                        // height bullet (высота пули)
    freqSFire = 20;                                         // freq speed bullet (чвстота доп. выстрелов)

// asteroid
let asteroY = -100,                                         // asteroid position y
    asteroDX = 1,                                           // change asteroid x (изменение положение пули по x)
    asteroDY = 1,                                           // change asteroid y (изменение положение пули по y)
    asteroWidth = 65,                                       // width asteroid (ширина пули) - st. 85
    asteroHeight = 65;                                      // height asteroid (высота пули) - st. 85

// health info
let healthX = 20,                                           // health position x
    healthY = 75,                                           // health position y
    healthWidth = 25,                                       // width health (ширина пули) - st. 25
    healthHeight = 22,                                      // height health (высота пули) - st. 22
    quanituHealth = 3;                                      // quanity health (количество жизней) - st. 3

// heart
let heartY = -100,                                          // heart position y
    heartWidth = 25,                                        // width heart (ширина пули) - st. 25
    heartHeight = 22,                                       // height heart (высота пули) - st. 22
    heartDX = 1,                                            // change heart x (изменение положение пули по x)
    heartDY = 1;                                            // change heart y (изменение положение пули по y)

// player (игрок)
let playerShip = {
    x : (width / 2) - (100 / 2),                            // payer ship posX
    y : (height / 2) - (60 / 2),                            // payer ship posY
    width : 100,                                            // payer ship width (standart - 85)
...
function createFire() {
    if (isMouseDown && timer % freqSFire === 0) {
        ...
        // bullet 1
        fire.push({
            x : (playerShip.x + playerShip.width / 2) - 5,      // astero posX
            y : playerShip.y,                                   // astero posY
            dx : fireDX,                                        // astero dx
            dy : fireDY,                                        // astero dx 
            width : fireWidth,                                  // astero width  (standart - 10px)
            height : fireHeight,                                // astero height (standart - 45px)
        });
    } 
};
...
function checkCollision() {
    ...
    for (i in astero) {
        if (
        (astero[i].x + astero[i].width/1.4 >= playerShip.x) && (astero[i].x <= playerShip.x + playerShip.width/1.4) &&
        (astero[i].y + astero[i].height/1.4 >= playerShip.y) && (astero[i].y <= playerShip.y + playerShip.height/1.4)) 
        {
    ...
    for (i in hearts) {
        if (
            (hearts[i].x + hearts[i].width >= playerShip.x) && (hearts[i].x <= playerShip.x + playerShip.width/1.4) &&
            (hearts[i].y + hearts[i].height >= playerShip.y) && (hearts[i].y <= playerShip.y + playerShip.height/1.4)
        ) 
        {   
...
function localStorageProcessing(action, nameStorge, data) {
    if (action === 'set') {
        localStorage.setItem(nameStorge, JSON.stringify(data));
    } else if ('get') {
        localStorage.getItem(nameStorge, JSON.parse(data));
    } else {
        console.error('Such method does not exixt');
    }
}
...
function moveAstero(minSX, maxSX, minSY, maxSY, freq) {  // freq - частота
    if (timer % freq == 0) {
        astero.push({
            x : (Math.random() * ((width - asteroWidth) - 1) + 1),       // astero posX
            y : asteroY,                                                 // astero posY
            dx : asteroDX,                                               // astero dx
            dy : asteroDY,                                               // astero dx 
            width : asteroWidth,                                         // astero width  
            height : asteroHeight,                                       // astero height 
            speedX : Math.random() * (maxSX - minSX + 1) + minSX,        // astero speedX
            speedY : Math.random() * (maxSY - minSY + 1) + minSY,        // astero speedY  
            angle : 0,
            del : false
        });
    }
    for (i in astero) {
        ...
        for(j in fire) {
            if ( 
                Math.abs(astero[i].x+astero[i].width/2 - fire[j].x-fire[j].width/2) < astero[i].width &&
                Math.abs(astero[i].y - fire[j].y) < astero[i].height) {
